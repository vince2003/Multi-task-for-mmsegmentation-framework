...
if not os.path.exists('./result_bw'):
    os.makedirs('./result_bw')
def main():
    args = parse_args()
    ...
    #     Image_pill=Image.fromarray(np.uint8(img*255))
    #     i=i+1
    #     Image_pill.save(f'result_bw/{i}.jpg')
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            Image_pill=Image.fromarray(np.uint8(result[0]*255))
            Image_pill.save(f'result_bw/{name}')
            #pdb.set_trace()
